...
public class RentCar extends HttpServlet {
    ...
    private void rentcar(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            String scapacity = request.getParameter("scapcity");
            ...
            String sql = "INSERT INTO rentcar (company_name, vehicle_name, mileage, transmission, fuel_type, sitting_capacity, price) VALUES (?, ?, ?, ?, ?, ?, ?)";
            ...
            ps.setString(6, scapacity);
            ps.setString(7, eprice);
...
//            smt.executeUpdate("UPDATE rentcar SET id = @newSerial:=@newSerial+1 ORDER BY id");
    ...
    private void rentcarimg(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        PrintWriter out = response.getWriter();
        HttpSession hs = request.getSession();
        ...
        String filePath = "C:\\Users\\neera\\OneDrive\\Documents\\NetBeansProjects\\Vehicle Valley\\web\\assests\\images\\rentcar\\";
...
//                try (Connection cn = DriverManager.getConnection("jdbc:mysql://localhost:3306/vehiclevalley", "root", ""); Statement smt = cn.createStatement()) {
                try (Connection cn = CreateConnection.getConnection(); Statement smt = cn.createStatement()) {
                    ...
                    int vid = 1;
                    ResultSet rs = smt.executeQuery("SELECT MAX(id) FROM rentcar");
                    if (rs.next()) {
                        vid = rs.getInt(1);
                    }
                    ...
                    while (i.hasNext()) {
                        ...
                        if (!fi.isFormField()) {
                            ...
                            int ii = smt.executeUpdate("UPDATE rentcar SET image_name='" + fn + "' WHERE id=" + vid);
                            if (ii >= 0) {
