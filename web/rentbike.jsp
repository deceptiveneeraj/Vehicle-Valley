<%-- 
    Document   : rentbike
    Created on : 28-Feb-2024, 9:45:59 pm
    Author     : neera
--%>
    ...
    <body>
        ...
        <div class="container mt-5">
            <div class="row row-cols-2 row-cols-md-2 row-cols-lg-3">
                <%!int vid; String cname, vname, mileage, ftype, fcapacity, eprice, img;%>
                <% try {
                        Connection cn = CreateConnection.getConnection();
                        Statement smt = cn.createStatement();
                        ResultSet rs = smt.executeQuery("select * from rentbike");
                        while (rs.next()) {
                            vid = rs.getInt(1);
                            ...
                            fcapacity = rs.getString(6);
                            eprice = rs.getString(7);
                            img = rs.getString(8);
                %>
                <div class="col" style="margin-top: 30px;">
                    <div class="card h-100">
                        <img src="assests/images/rentbike/<%= img%>" class="card-img-top" alt="Vehicle Image">
                        <div class="card-body">
                            <h5 class="card-title"><%=cname%> <%=vname%></h5>
                            <p class="card-title"><b>Description:</b> Mileage <%=mileage%> kmpl, Fuel Type: <%=ftype%>, Sitting Capacity: <%=fcapacity%> L</p>
                            <h5 class="card-text">Rent Price:<b> <%= eprice%></b>  RS Per Day</h5>
                            <a href="booknow.jsp?type=Bike&id=<%=vid%>" class="btn btn-primary">Book Now</a>
                        </div>
                    </div>
                </div>
                <% }
